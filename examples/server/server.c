...
static WC_INLINE int TranslateReturnCode(int old, int sd) {
  ...
  if (old < 0) {
    ...
    if (errno == RTCSERR_TCP_CONN_RLSD) errno = SOCKET_ECONNRESET;
    if (errno == RTCSERR_TCP_TIMED_OUT) errno = SOCKET_EAGAIN;
  }
#endif
...
int ServerEchoData(SSL* ssl, int clientfd, int echoData, int block,
          ...
          if (err == WC_PENDING_E) {
            ...
            if (ret < 0) break;
              ...
              if (err != WOLFSSL_ERROR_WANT_READ &&
                  err != WOLFSSL_ERROR_ZERO_RETURN && err != APP_DATA_READY) {
  ...
  if (throughput) {
    printf(
        ...
        (int)throughput,
#endif
        rx_time * 1000, throughput / rx_time / 1024 / 1024, tx_time * 1000,
        throughput / tx_time / 1024 / 1024);
  }
    ...
    if (ret < 0) {
      ...
      if (err == APP_DATA_READY) {
        ...
        if (ret >= 0) {
          ...
          input[ret] = '\0';
          printf(
              "Client message received during "
...
static void SetKeyShare(WOLFSSL* ssl, int onlyKeyShare, int useX25519,
  ...
  if (onlyKeyShare == 2) {
    if (useX25519) {
      ...
      do {
        ...
        if (ret == WOLFSSL_SUCCESS) groups[count++] = WOLFSSL_ECC_X25519;
#ifdef WOLFSSL_ASYNC_CRYPT
      ...
      do {
        ...
        if (ret == WOLFSSL_SUCCESS) groups[count++] = WOLFSSL_ECC_X448;
#ifdef WOLFSSL_ASYNC_CRYPT
        else if (ret == WC_PENDING_E)
          wolfSSL_AsyncPoll(ssl, WOLF_POLL_FLAG_CHECK_HW);
#endif
      ...
      } else if (XSTRNCMP(pqcAlg, "KYBER_LEVEL3", XSTRLEN("KYBER_LEVEL3")) ==
                 0) {
        groups[count] = WOLFSSL_KYBER_LEVEL3;
      } else if (XSTRNCMP(pqcAlg, "KYBER_LEVEL5", XSTRLEN("KYBER_LEVEL5")) ==
                 0) {
      ...
      } else if (XSTRNCMP(pqcAlg, "SABER_LEVEL1", XSTRLEN("SABER_LEVEL1")) ==
                 0) {
        groups[count] = WOLFSSL_SABER_LEVEL1;
      } else if (XSTRNCMP(pqcAlg, "SABER_LEVEL3", XSTRLEN("SABER_LEVEL3")) ==
                 0) {
        groups[count] = WOLFSSL_SABER_LEVEL3;
      } else if (XSTRNCMP(pqcAlg, "SABER_LEVEL5", XSTRLEN("SABER_LEVEL5")) ==
                 0) {
      ...
      do {
        ...
        if (ret == WOLFSSL_SUCCESS) groups[count++] = WOLFSSL_ECC_SECP256R1;
#ifdef WOLFSSL_ASYNC_CRYPT
        else if (ret == WC_PENDING_E)
          wolfSSL_AsyncPoll(ssl, WOLF_POLL_FLAG_CHECK_HW);
#endif
  ...
  if (onlyKeyShare == 1) {
    ...
    do {
      ...
      if (ret == WOLFSSL_SUCCESS) groups[count++] = WOLFSSL_FFDHE_2048;
#ifdef WOLFSSL_ASYNC_CRYPT
      else if (ret == WC_PENDING_E)
        wolfSSL_AsyncPoll(ssl, WOLF_POLL_FLAG_CHECK_HW);
#endif
  ...
  if (count >= MAX_GROUP_NUMBER) err_sys("example group array size error");
  if (count > 0) {
...
/* 55 */
    ...
    !defined(NO_FILESYSTEM) && !defined(NO_WOLFSSL_DIR)
        ...
        "            files in the folder would have the form \"hash.N\" file "
        "name\n"
        "            e.g symbolic link to the file at certs folder\n"
        "            ln -s client-ca.pem  `openssl x509 -in client-ca.pem "
        "-hash -noout`.0\n",
/* 57 */
#endif
...
/* 58 */
        ...
        "            P256_SABER_LEVEL1, P384_SABER_LEVEL3, P521_SABER_LEVEL5, "
        "P256_KYBER_LEVEL1,\n"
...
/* 58 */
#endif
#ifndef WOLFSSL_TLS13
        "-7          最小ダウングレード可能なプロトコルバージョンを設定します "
        "[0-3] "
        " SSLv3(0) - TLS1.2(3)\n",
#else
        "-7          最小ダウングレード可能なプロトコルバージョンを設定します "
        ...
        "[KYBER_LEVEL1, KYBER_LEVEL3, KYBER_LEVEL5, KYBER_90S_LEVEL1, "
        "KYBER_90S_LEVEL3, KYBER_90S_LEVEL5,\n",
        " NTRU_HPS_LEVEL1, NTRU_HPS_LEVEL3, NTRU_HPS_LEVEL5, "
        "NTRU_HRSS_LEVEL3,\n",
        ...
        " P256_SABER_LEVEL1, P384_SABER_LEVEL3, P521_SABER_LEVEL5, "
        "P256_KYBER_LEVEL1,\n"
        " P384_KYBER_LEVEL3, P521_KYBER_LEVEL5, P256_KYBER_90S_LEVEL1, "
...
/* big enough to handle most cases including session certs */
  ...
  if (method == NULL) err_sys_ex(runWithErrors, "unable to get method");
  ...
  if (wolfSSL_CTX_load_static_memory(&ctx, method, memory, sizeof(memory), 0,
                                     1) != WOLFSSL_SUCCESS)
    err_sys_ex(catastrophic, "unable to load static memory and create ctx");

  /* load in a buffer for IO */
  if (wolfSSL_CTX_load_static_memory(
  ...
  if (ctx == NULL) err_sys_ex(catastrophic, "unable to get ctx");
  ...
  if (wolfSSL_CTX_set_AcceptFilter(
          ctx, (NetworkFilterCallback_t)wolfSentry_NetworkFilterCallback,
          wolfsentry) < 0) {
  ...
  ret = wolfSSL_CTX_set_ephemeral_key(ctx, WC_PK_TYPE_ECDH,
                                      "./certs/statickeys/ecc-secp256r1.pem", 0,
  ...
  ret = wolfSSL_CTX_set_ephemeral_key(ctx, WC_PK_TYPE_DH,
                                      "./certs/statickeys/dh-ffdhe2048.pem", 0,
                                      WOLFSSL_FILETYPE_PEM);
  if (ret != 0) {
  ...
  if (fewerPackets) wolfSSL_CTX_set_group_messages(ctx);
#if (defined(WOLFSSL_SCTP) || defined(WOLFSSL_DTLS_MTU)) && \
    defined(WOLFSSL_DTLS)
  if (dtlsMTU) wolfSSL_CTX_dtls_set_mtu(ctx, dtlsMTU);
#endif

#ifdef WOLFSSL_SCTP
  if (dtlsSCTP) wolfSSL_CTX_dtls_set_sctp(ctx);
#endif
  ...
  if ((!usePsk || usePskPlus) && !useAnon && !(loadCertKeyIntoSSLObj == 1)) {
    ...
    if (wolfSSL_CTX_use_certificate_chain_buffer_format(
            ctx, server_cert_der_2048, sizeof_server_cert_der_2048,
...
#ifndef NO_CERTS
  if (mutualAuth) wolfSSL_CTX_mutual_auth(ctx, 1);
#endif
  ...
  if (doCliCertCheck && (usePsk == 0 || usePskPlus) && useAnon == 0) {
    ...
    if (useCertFolder) {
      ...
      if (wolfSSL_CTX_load_verify_locations_ex(
              ctx, verifyCert, 0, verify_flags) != WOLFSSL_SUCCESS) {
        err_sys_ex(catastrophic,
                   "can't load ca file, Please run from wolfSSL home dir");
      }
#ifdef WOLFSSL_TRUST_PEER_CERT
      if (trustCert) {
        if ((ret = wolfSSL_CTX_trust_peer_cert(
                 ctx, trustCert, WOLFSSL_FILETYPE_PEM)) != WOLFSSL_SUCCESS) {
...
#ifdef HAVE_SESSION_TICKET
#ifdef WOLFSSL_TLS13
  if (noTicketTls13) wolfSSL_CTX_no_ticket_TLSv13(ctx);
#endif
#if !defined(WOLFSSL_NO_TLS12) || !defined(NO_OLD_TLS)
  if (noTicketTls12) wolfSSL_CTX_NoTicketTLSv12(ctx);
#endif
#endif
  ...
  while (1) {
...
#ifdef HAVE_PK_CALLBACKS
    if (pkCallbacks) SetupPkCallbacks(ctx);
#endif

    ssl = SSL_new(ctx);
    if (ssl == NULL) err_sys_ex(catastrophic, "unable to create an SSL object");
    ...
    if ((!usePsk || usePskPlus) && !useAnon && loadCertKeyIntoSSLObj) {
      ...
      if (wolfSSL_use_certificate_chain_buffer_format(
              ssl, server_cert_der_2048, sizeof_server_cert_der_2048,
              WOLFSSL_FILETYPE_ASN1) != WOLFSSL_SUCCESS)
        err_sys_ex(catastrophic, "can't load server cert buffer");
#elif !defined(TEST_LOAD_BUFFER)
...
#define PMS_SZ 512                     /* ENCRYPT_LEN (see internal.h) */
...
#ifdef WOLFSSL_TRUST_PEER_CERT
      if (trustCert) {
        if ((ret = wolfSSL_trust_peer_cert(
                 ssl, trustCert, WOLFSSL_FILETYPE_PEM)) != WOLFSSL_SUCCESS) {
          err_sys_ex(runWithErrors,
    ...
    if (useOcsp) {
      if (ocspUrl != NULL) {
        ...
        wolfSSL_CTX_EnableOCSP(
            ctx, WOLFSSL_OCSP_NO_NONCE | WOLFSSL_OCSP_URL_OVERRIDE);
      } else
...
#ifdef HAVE_ENCRYPT_THEN_MAC
    if (disallowETM) wolfSSL_AllowEncryptThenMac(ssl, 0);
#endif
    ...
    client_len = sizeof client_addr;
    tcp_accept(&sockfd, &clientfd, (func_args*)args, port, useAnyAddr, dtlsUDP,
               dtlsSCTP, serverReadyFile ? 1 : 0, doListen, &client_addr,
               &client_len);
    ...
    if (doDTLS && dtlsUDP) {
      ...
      if (n <= 0) err_sys_ex(runWithErrors, "recvfrom failed");
...
#ifdef WOLFSSL_WOLFSENTRY_HOOKS
    {
      ...
      if (wolfsentry_store_endpoints(ssl, &client_addr, &local_addr,
    ...
    !defined(HAVE_FIPS) && !defined(HAVE_SELFTEST)
      if (!doDhKeyCheck) wolfSSL_SetEnableDhKeyTest(ssl, 0);
#endif
    }
    ...
    if (cipherList && !useDefCipherList && (!XSTRSTR(cipherList, ":"))) {
      ...
      if (established_cipher &&
          ...
          (wolfSSL_get_cipher_suite_from_name(
               cipherList, &requested_cipherSuite0, &requested_cipherSuite,
        ...
        if (established_cipher_name == NULL)
          err_sys_ex(catastrophic,
                     "error looking up name of established cipher");
        ...
        if (strcmp(cipherList, established_cipher_name) &&
          ...
          if (!(requested_cipherFlags & WOLFSSL_CIPHER_SUITE_FLAG_NAMEALIAS))
            err_sys_ex(catastrophic,
                       "Unexpected mismatch between names of requested and "
                       "established ciphers.");
            ...
            do {
              ...
              if ((ret = wolfSSL_accept(ssl)) != WOLFSSL_SUCCESS) {
                err = wolfSSL_get_error(ssl, ret);
                     ...
                     (err == WOLFSSL_ERROR_WANT_READ ||
                      err == WOLFSSL_ERROR_WANT_WRITE ||
                      err == APP_DATA_READY || err == WC_PENDING_E));
    ...
    if (err == 0 && echoData == 0 && throughput == 0) {
...
#ifdef WOLFSSL_TLS13
      if (updateKeysIVs) wolfSSL_update_keys(ssl);
#endif
#if defined(WOLFSSL_TLS13) && defined(WOLFSSL_POST_HANDSHAKE_AUTH)
      if (postHandAuth) wolfSSL_request_certificate(ssl);
#endif
      ...
      if (err == WOLFSSL_ERROR_ZERO_RETURN && exitWithRet == 0) err = 0;
      if (err != 0) {
        SSL_free(ssl);
        ssl = NULL;
        SSL_CTX_free(ctx);
    ...
    if (wc_shutdown && ret == WOLFSSL_SHUTDOWN_NOT_DONE) {
      ...
      if (ret == WOLFSSL_SUCCESS) printf("Bidirectional shutdown complete\n");
    }
    ...
    fprintf(stderr,
            "*** This is memory state before wolfSSL_free is "
            "called\n");
...
THREAD_RETURN WOLFSSL_THREAD server_test2(void* args, char* response,
...
#define PMS_SZ 512                     /* ENCRYPT_LEN (see internal.h) */
    ...
    // cpu時間
    long cpu_time_total = clock();  // ハンドシェイク完了
    long cpu_time_rest = cpu_time_total - cpu_time_rrcend;
    // rrcend
    fp = fopen("eval_cpu_rrcend.txt", "a");
    fprintf(fp, "%ld\n", cpu_time_rrcend);
    fclose(fp);
    // rest
    fp = fopen("eval_cpu_rest.txt", "a");
    fprintf(fp, "%ld\n", cpu_time_rest);
    fclose(fp);
    // total
    fp = fopen("eval_cpu_total.txt", "a");
    fprintf(fp, "%ld\n", cpu_time_total);
    fclose(fp);
